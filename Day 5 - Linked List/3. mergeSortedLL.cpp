...
Node* mergeRecursive(Node* l1, Node* l2){
    ...
    if(l1->data < l2->data){
        l = l1;
        l->next = mergeRecursive(l1->next, l2);
    } else {
        l = l2;
        l->next = mergeRecursive(l1, l2->next);
    }
...
int main() {
    ...
    // call either iterative or recursive at a time
    Node* iter = mergeIterative(l1, l2);
    printLL(iter);
    // Node* rec = mergeRecursive(l1, l2);
    // printLL(rec);
